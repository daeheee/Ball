...
public:
	...
	;
private:
	double CalLength(double x, double y);
	double GetTan(double x, double y);

protected:
	Room* room_;
    const double PI_= 57.2957951; // 180/pi 각도 값을 구하기 위한 값
	...
	// position
	double p_[2];	// x, y
	double p_o[2];  // orbit x,y
	// velocity
	double v_[2];	// x, y
	double v_o[2];  // orbit x,y
